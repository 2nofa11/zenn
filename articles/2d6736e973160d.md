...
この記事は、VueUseの `useIntersectionObserver` フックを使って簡単なWebアプリをハンズオンで作成しています！
...
必要な Vue の機能（`ref`, `onMounted`）の説明は冗長になるため割愛します。
またコードは`<script setup>`を使用した記述になっています。これらはVue.js公式ドキュメントに詳細が載っています！
...
公式の以下を参考に今回使いたい`useIntersectionObserver`をインポートします。以降は`<script setup>`内のコードだと思ってください。
...
またマウント時に`loadMoreImages`を実行してあげてるようにしてあげましょう。
...
また、Tailwind.cssを使って、若干のスタイリングを施しています。
