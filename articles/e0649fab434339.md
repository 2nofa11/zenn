...
[Vue Advent Calendar 2025](https://qiita.com/advent-calendar/2025/vue) 6 日目の記事です。
...
`TanStack Form`は公式で以下の記載がある通り、型安全なフォームのヘッドレスUIライブラリです。

> `Headless UI for building performant and type-safe forms`（高性能で型安全なフォームを構築するためのヘッドレスUI）
...
名前を登録するフォームをサンプルとします。
* バリデーション条件
    * サブミット時、ブラー時で検知
* バリデーション内容
    * 空文字入力の場合は「名前は必須です」というエラーを表示する
...
#### `useForm`
  ...
  まず、`@tanstack/vue-form`からimportした`useForm`を利用しています。宣言的にフォームの情報を書くことができます。[^1]
...
素のVueで書くより覚えることが多いと感じましたが、処理の流れが`useForm`を通して一方通行になっている点は良いと感じました。
